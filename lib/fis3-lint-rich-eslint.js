...
module.exports = function(content, file, conf) {
    ...
    fixedText = report.results[firstIndex].output;

    // 输出修复过的文件到根目录/lint-fixed
    if (mixinConfig.fix && mixinConfig.allowOutfixed && fixedText) {
        let dirname = path.join(process.cwd(), '/lint-fixed', file.subdirname);
        // 创建或检测是否存在文件所在目录
