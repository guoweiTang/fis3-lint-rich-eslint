...
// 设置lint之后的异常条数，通过fis.get(name)获取
fis.set('ESLINT_RESULT', {});
fis.set('ESLINT_RESULT.errCount', 0);
fis.set('ESLINT_RESULT.totalCount', 0);
...
module.exports = function (content, file, conf) {
    let mixinConfig = {
        fix: false,
        allowOutfixed: false,
        envs: Object.keys(lagouConfig.env),
        globals: [
            '__inline',
            '__uri',
            '__RESOURCE_MAP__',
    ...
    if (conf) {
        if (conf.envs && conf.envs.length) {
            mixinConfig.envs = conf.envs;
            delete conf.envs;
        }
    ...
    if (report.errorCount || report.warningCount) {
        ...
        // 导出异常信息条数
        fis.set('ESLINT_RESULT.errCount', result.errCount);
        fis.set('ESLINT_RESULT.totalCount', result.totalCount);
        return;
    }
